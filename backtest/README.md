...
# 输出：
# - backtest/data/historical-cache.json（~18MB，覆盖2019-01至今）
# - backtest/data/cache-meta.json（元信息）
```
...
```bash
# 2. 批量回测（随机采样50个时间点）
node backtest/batch-runner.cjs \
  --start 2019-01-01 \
  --end 2026-07-31 \
  --sample-count 50 \
  --sample-mode random \
...
# 输出：
# - backtest/data/backtest-log.jsonl（增量日志，每完成1次追加1行）
# - backtest/data/backtest-summary.json（汇总统计）
```
...
```bash
# 3. 查看汇总报告
cat backtest/data/backtest-summary.json
...
回测验证路径（离线）：
  backtest/data/historical-cache.json (7-12年全历史)
