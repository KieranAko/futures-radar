// legacy/strategies/signal-backtest/build-probability-cone-v7.cjs — 用生产概率模块为回测锚点计算真实概率锥
//
// 目的：把“定价依据”还回生产链路——strategy-matcher 的 targets 需要 probability.json 的
// HV 概率锥；回测不能再用手写 ATR 代理。本脚本对每个锚点截断 bars，调用生产
// analysis/probability/hv-estimators（Yang-Zhang）与 analysis/probability/probability-cone.js（GBM 闭式）。
'use strict';
...
async function main() {
  const { probabilityCone } = await import('../../../analysis/probability/probability-cone.js');
  const { yangZhangVolatility } = await import('../../../analysis/probability/hv-estimators.js');
