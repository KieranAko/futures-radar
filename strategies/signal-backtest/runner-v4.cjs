// strategies/signal-backtest/runner-v4.cjs — v4 微型试点：上下文 + FinCoT 三臂消融
//
// 三臂（同一批锚点日期、同一执行引擎）：
//   A 纯价格（v3 冻结锚点）
...
function loadBars(symbol) {
  ...
  const fixture = path.join(V4, 'history-2y.json');
  if (fs.existsSync(fixture)) {
    const j = loadJSON(fixture);
    return { bars: j.symbols[symbol].bars, source: 'recordings/v4/history-2y.json fixture' };
  }
  throw new Error(`runner-v4: no bars for ${symbol}`);
}
...
function loadPlans(arm) {
  if (arm === 'A') {
    const j = loadJSON(path.join(V4, 'arm-A.json'));
    return Object.fromEntries(SYMBOLS.map(s => [s, j.symbols[s]]));
  }
  const out = {};
  for (const sym of SYMBOLS) {
    const j = loadJSON(path.join(V4, `arm-${arm}-${sym}.json`));
    out[sym] = j.anchors;
  }
  return out;
}
...
function renderMd(report) {
  const L = [];
  L.push('# 信号质量回测基线 v4（上下文 + FinCoT 三臂试点）');
  L.push('');
  L.push('> 试点范围：RB0/M0/SC0 × 最近 10 个锚点（2026-06-11 .. 2026-08-14，5 日间隔），验证到 2026-08-27，固定 1 手。');
...
function main() {
  ...
  falsification.push('执行引擎对三臂完全一致；三臂差异只来自 LLM 决策上下文。30 个锚点、固定 1 手，仅作试点证据。');

  const report = {
    schema: 'futures-radar-signal-backtest/4',
    meta: {
      generatedAt: new Date().toISOString(),
      arms: ARMS,
      universe: SYMBOLS,
      anchorsPerSymbol: 10,
      barsRange: '2026-06-11..2026-08-27（计划窗口）',
    ...
    signals: Object.fromEntries(ARMS.map(arm => [arm, arms[arm].signals.map(s => ({
      ...
      finCotAlignment: s.finCotAlignment, qualityFlags: s.qualityFlags || [], contextRefs: s.contextRefs || [],
  ...
  const jsonPath = path.join(OUT_DIR, 'signal-quality-baseline-v4.json');
  const mdPath = path.join(OUT_DIR, 'signal-quality-baseline-v4.md');
