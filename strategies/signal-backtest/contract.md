# signal-backtest 信号质量回测契约（v1）

## 目标
只测「信号 → 策略计划 → 证伪」这条链路的信号质量，不测筛选/扫描：
- 品种固定：RB0、M0、SC0
- 历史长度：约 1 年（250 个交易日）
- LLM 锚点：每 10 个交易日做一次方向/参数决策（只允许看 `bars[0..anchorIdx]`）
...
strategies/signal-backtest/
  contract.md                      本契约
  runner.cjs                       回测运行器（确定性，读取 recordings）
  recordings/
    features.json                  每个锚点的截断特征（只含锚点日及以前的数据）
    anchors-RB0.json               LLM 锚点决策（人工录制/LLM 产出）
    anchors-M0.json
    anchors-SC0.json
  output/
    signal-quality-baseline.json    结果
    signal-quality-baseline.md      结果
```

## LLM 锚点决策 schema（anchors-<SYM>.json）
```json
{
  "symbol": "RB0",
  "step": 10,
...
## 验证指标（output/signal-quality-baseline.json）
- meta：universe、barsRange、anchorStep、anchorCount、signalCount、verifiedCount。
...
- 证伪结论：哪种参数组合（triggerAtrMult/stopAtrMult/targetR/maxHoldDays）方向正确率最低，作为下一轮调参依据。
