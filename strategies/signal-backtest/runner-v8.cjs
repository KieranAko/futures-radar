// strategies/signal-backtest/runner-v8.cjs — 按生产 strategy-plan.json 执行（FinCoT→策略库→执行）
//
...
function executePlan(symbol, planFile) {
  ...
  if (anchorIdx === -1 || p.executionStatus !== 'executable' || p.reportBaseline.direction === 'neutral') {
...
function main() {
  ...
  const results = files.map(f => {
    const sym = f.split('-')[0];
    return executePlan(sym, path.join(planDir, f));
  });
  ...
  const report = {
    schema: 'futures-radar-signal-backtest/8',
    meta: {
  ...
  const jsonPath = path.join(OUT_DIR, 'signal-quality-baseline-v8.json');
  const mdPath = path.join(OUT_DIR, 'signal-quality-baseline-v8.md');
  fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2), 'utf8');
  const L = ['# 信号质量回测基线 V8（生产 strategy-plan 执行）', '',
    '> 链路：V7 FinCoT（分析）→ 生产 strategy-matcher（策略适配）→ 本执行引擎（只读 plan 字段）。', '',
    `- 30 个计划：executable ${agg.executable}、成交 ${agg.verifiedCount}、方向正确率 ${agg.directionCorrectRate}%、毛 ${agg.avgPnlPct}%/净 ${agg.avgNetPnlPct}%`, '',
    '| 品种 | 计划 | executable | 成交 | 方向正确率 | 毛盈亏 | 净盈亏 |',
    '|---|---|---|---|---|---|---|'];
  for (const sym of SYMBOLS) {
    const a = bySymbol[sym];
    L.push(`| ${sym} | ${a.plans} | ${a.executable} | ${a.verifiedCount} | ${a.directionCorrectRate}% | ${a.avgPnlPct}% | ${a.avgNetPnlPct}% |`);
  }
