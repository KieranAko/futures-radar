// strategies/story-pool/build-filtered-from-story-pool.cjs — V2 初筛替代品
//
...
//   node strategies/story-pool/build-filtered-from-story-pool.cjs --runId <runId>
//
// 行为：
...
const { runDir, skillRoot } = require('../../lib/workspace.cjs');
...
function patchCandidatesFile(candidatesPath, entries, rawJson = {}) {
...
function main() {
  ...
  patchCandidatesFile(path.join(dir, 'candidates.json'), filtered.candidates, rawJson);
...
module.exports = { main, buildFilteredFromStoryPool, storySeatEntries, trackingSeatEntries };
