// strategies/lib/story-chain.cjs — 传导链构造器核心（故事池 v2）
//
// 职责边界：
...
const CHAIN_SCHEMA = 'futures-radar-story-chain/2';
...
function loadCatalog() {
  return readJson(path.join(skillRoot, 'config', 'story-chain-indicators.json'), null);
}
...
function validateChainDefinition(def, { catalog = null, symbols = null } = {}) {
  ...
  if (def.schema !== CHAIN_SCHEMA && def.schema !== LEGACY_CHAIN_SCHEMA) {
    errors.push(`schema 必须为 ${CHAIN_SCHEMA}（兼容 ${LEGACY_CHAIN_SCHEMA}）`);
  }
