...
| `entry` | object | ✓ | trigger（含转执行触发=Q4 确认价位）、triggerLevel（Q4 第一个数字，无则 null）、triggerSource（Q4 原文）、**triggerTiming（触发/执行时点语义：收盘确认后下一交易日开盘执行，或 T+1 开盘执行；neutral=无执行时点）**、execution（执行口径+执行偏离阈值） |
