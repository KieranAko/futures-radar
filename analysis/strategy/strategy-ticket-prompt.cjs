...
function main() {
  ...
  L.push('4. 确认后怎么入场，什么情况放弃不入；放弃条款必须给出具体点数（写“偏离 >X 放弃”，X 是点数），不要写需要临场心算的表达式；入场执行区间必须写明两条边（写“入场区间 X–Y”，X/Y 是具体价位），并分别说明每一边的依据（哪条边是触发条件、哪条边是放弃条件）；');
  L.push('5. 错了在哪止损；');
  L.push('6. 对了看哪两个目标；');
  L.push('7. 最长持有：必须写成绝对交易日，例如“T+N 交易日收盘前未到目标则离场”，并在括号里注明入场后实际持有几个交易日；');
  L.push('8. 什么情况直接作废这单。');
  L.push('');
  L.push('时间一律用交易日表述：T+N 表示信号确认后的第 N 个交易日；不要用自然日，也不要用“分钟K线收盘”这类说法。');
  L.push('');
