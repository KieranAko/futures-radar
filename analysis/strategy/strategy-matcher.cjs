// analysis/strategy/strategy-matcher.cjs — Stage 8 (t8): 确定性 TOP3 策略匹配引擎
//
...
const RULES_PATH = () => path.join(skillRoot, 'analysis', 'strategy', 'strategy-matching-rules.json');
const PLAN_SCHEMA_PATH = () => path.join(skillRoot, 'analysis', 'strategy', 'strategy-plan.schema.json');
...
function buildStrategyPlan({ runId, equityCny = 100000, reasoning = null, elements = null, volTargetPerPosition = null }) {
  ...
  const plan = {
    ...
    provenance: {
      ...
      generator: 'futures-radar/analysis/strategy/strategy-matcher.cjs',
      discipline: [
