// analysis/v2/prompt-builder-v2.cjs — O1/O2/O3/O5：单轮合并 prompt 构建
//
// 输出两块 prompt（2 次逻辑 LLM 调用，目标 ≤3）：
//   P1 板块批量：全部相关板块一次判定（O2）
//   P2 品种批量：三品种一次输出六问 + mechanismRef + selfCheck（O1/O3/O5）
//
// 用法: node analysis/v2/prompt-builder-v2.cjs --runId <runId>
'use strict';
...
function main() {
  ...
  for (const sym of syms) {
    ...
    if (nt) {
      ...
      L.push(`- near_term: close=${nt.close} atr5=${nt.atr5} pdh=${nt.pdh}(${f(d.pdh)}) pdl=${nt.pdl}(${f(d.pdl)}) h3=${nt.h3}(${f(d.h3)}) l3=${nt.l3}(${f(d.l3)}) vaH=${nt.valueAreaHigh}(${f(d.valueAreaHigh)}) vaL=${nt.valueAreaLow}(${f(d.valueAreaLow)})`);
    }
    L.push('');
  }
  L.push('');
