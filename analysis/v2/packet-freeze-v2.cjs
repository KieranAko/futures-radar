// analysis/v2/packet-freeze-v2.cjs — O4/O5 前置：确定性冻结 packet（无网络）
//
...
//
// 用法: node analysis/v2/packet-freeze-v2.cjs --runId <runId>
'use strict';
...
const basisLib = require(path.join(ROOT, 'legacy', 'strategies', 'research', 'v2', 'falsification', 'harness-lib', 'basis.cjs'));
const { computeNearTermStructure } = require(path.join(ROOT, 'analysis', 'strategy', 'near-term-structure.cjs'));
...
function main() {
  ...
  // 成本锚：只从当期快照读取（快照由文件库主档投影，见 analysis/v2/cost-anchor/）
  const costAnchorMap = {};
