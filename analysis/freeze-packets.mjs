/**
 * analysis/freeze-packets.mjs — Analyze 阶段步骤 1（自动）
 ...
 *
 * Usage:
 *   node analysis/freeze-packets.mjs --runId 20260825-1020-auto
 */

import fs from 'node:fs';
import path from 'node:path';
import { createRequire } from 'node:module';
import { buildPacketFromRawJson } from '../legacy/reasoning/reasoning/lib/raw-adapter.js';
import { buildPacket } from '../legacy/reasoning/reasoning/lib/packet-builder.js';
import { buildPacketBundle } from '../legacy/reasoning/reasoning/lib/packet-bundle.js';
import { extractTermStructure } from '../legacy/reasoning/reasoning/lib/term-structure.js';
import { fetchContractHistory, overrideWithCleanSeries } from '../legacy/reasoning/reasoning/lib/specific-contract.js';
import { renderFourArmPrompts } from '../legacy/reasoning/reasoning/lib/prompt-renderer.js';
import { buildMacroContext } from '../legacy/reasoning/reasoning/lib/macro-context.js';

const require = createRequire(import.meta.url);
const { runDir } = require('../shared/workspace.cjs');
const { validateMacroSnapshot } = require('../collection/macro-probe.cjs');
const { relevantAnchorsFor } = require('../analysis/assembly/build-facts.cjs');
const dataStore = require('../storage/index.cjs');
const { buildSectorField, buildSectorSnapshot } = require('../collection/sector-aggregator.cjs');
...
if (!sectorSnapshot || !sectorSnapshot.sectors) {
  try {
    const symbolsConfigPath = path.join(require('../shared/workspace.cjs').skillRoot, 'config', 'symbols.json');
