...
function buildStoryChainPrompt(ctx = {}) {
  ...
  L.push('  "chains": [');
  L.push('    {');
  L.push('      "schema": "futures-radar-story-chain/2",');
  L.push('      "chainId": "CH-<SECTOR>-<YYYYMMDD>-<NN>",');
  L.push('      "createdAt": "<YYYY-MM-DD>",');
  L.push('      "theme": "黑色流动性退潮",');
  L.push('      "themeDetail": "DR007 抬头收紧流动性，黑色资金持续流出，螺纹钢等待需求证伪",');
  L.push('      "sector": "black",');
  L.push('      "direction": -1,');
  ...
  L.push('      "nodes": [');
  L.push('        { "id": "n1", "indicatorId": "macro.DR007.change5d", "expectation": 1, "label": "流动性收紧" },');
  L.push('        { "id": "n2", "indicatorId": "sector.black.oi.flow5d", "expectation": -1, "label": "黑色资金流出" },');
  L.push('        { "id": "n3", "indicatorId": "symbol.RB0.price.ret5d", "expectation": -1, "label": "螺纹转弱" }');
  L.push('      ],');
  L.push('      "edges": [');
  L.push('        { "id": "e1", "from": "n1", "to": "n2", "latencyDays": 5, "logic": "流动性收紧→黑色系需求敏感品种资金流出" },');
  L.push('        { "id": "e2", "from": "n2", "to": "n3", "latencyDays": 5, "logic": "资金流出→代表品种螺纹钢转弱" }');
  L.push('      ]');
  L.push('    }');
  L.push('  ]');
  L.push('}');
  L.push('```');
  L.push('');
  L.push('硬约束：节点 ≥3；边 = 节点数−1 且按顺序连接；指标必须在上节目录内；direction/expectation 仅 ±1；latencyDays 1..10；2 ≤ entryProofIndex < 节点数；theme 4–14 字且不含 →；themeDetail 10–60 字且不含 →；同板块一条活跃链（替换时 replace=true，旧链自动 superseded 留痕）。');
  L.push('');
