...
function buildStoryChainPrompt(ctx = {}) {
  ...
  L.push('5. 写主题：主标题 4–14 字（短语，禁止 →），副标题 10–60 字（驱动+传导+品种+阶段，禁止 →）；');
  L.push('6. 输出前自查四条标准：自洽、可监测、可证伪、未走完。');
  L.push('');
  L.push('## 输出契约');
  L.push('');
  ...
  L.push('      "sourceId": "macro.SC0.change5d",');
  L.push('      "theme": "原油坍塌的能化传导",');
  L.push('      "themeDetail": "SC0 成本坍塌向能化与航运成本传导，燃料油补跌尚未走完",');
  L.push('      "replace": false,');
  L.push('      "nodes": [');
  L.push('        { "id": "n1", "indicatorId": "macro.SC0.change5d", "expectation": -1, "label": "原油下跌" },');
  L.push('        { "id": "n2", "indicatorId": "sector.energy_chemical.oi.flow5d", "expectation": -1, "label": "能化资金流出" },');
  L.push('        { "id": "n3", "indicatorId": "symbol.FU0.price.ret5d", "expectation": -1, "label": "燃料油补跌", "terminal": true, "priority": "primary", "impactRationale": "燃料油对 SC0 成本弹性最大，且前期跌幅滞后，补跌空间最大", "proofIndex": 2 },');
  L.push('        { "id": "n4", "indicatorId": "symbol.PG0.price.ret5d", "expectation": -1, "label": "LPG 下行", "terminal": true, "priority": "secondary", "impactRationale": "LPG 成本支撑直接下移，但弹性弱于燃料油", "proofIndex": 2 }');
  L.push('      ],');
  L.push('      "edges": [');
  L.push('        { "id": "e1", "from": "n1", "to": "n2", "latencyDays": 5, "logic": "原油下跌→能化资金流出" },');
  L.push('        { "id": "e2", "from": "n2", "to": "n3", "latencyDays": 5, "logic": "能化资金流出→燃料油补跌" },');
  L.push('        { "id": "e3", "from": "n1", "to": "n4", "latencyDays": 7, "logic": "原油下跌→LPG 成本支撑下移" }');
  L.push('      ]');
  L.push('    }');
  L.push('  ]');
  L.push('}');
  L.push('```');
  L.push('');
  L.push('硬约束：schema /3；sourceId 等于首节点指标或概念，且同源同时只允许一条活跃链（replace=true 换代）；节点 ≥3；图必须无环（允许扇出/汇合）；terminal 必须是 symbol.* 可交易节点，priority=primary|secondary，impactRationale 8–80 字，2 ≤ proofIndex ≤ 祖先节点数；指标必须在上节目录内；expectation 仅 ±1；latencyDays 1..10；theme 4–14 字且不含 →；themeDetail 10–60 字且不含 →。');
  L.push('');
