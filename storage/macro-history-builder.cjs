// storage/macro-history-builder.cjs — 宏观历史序列入库（文件库唯一事实源）
//
// 数据来源（均为真实已抓取数据，不读 output/runs）：
//   1. 冻结宏观历史 legacy/strategies/signal-backtest/recordings/v4/macro-history.json
...
//
// 用法: node storage/macro-history-builder.cjs [--build]
'use strict';
...
const BASE_FILE = path.join(skillRoot, 'legacy', 'strategies', 'signal-backtest', 'recordings', 'v4', 'macro-history.json');
...
function buildMacroHistory() {
  ...
  for (const a of ANCHORS) {
    ...
    writeJsonAtomic(path.join(OUT_DIR, `${a}.json`), {
      ...
      source: 'legacy/strategies/signal-backtest/recordings/v4/macro-history.json + data/macro/<RUN_ID>.json',
      rows: merged.length,
