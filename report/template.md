...
**Four chapters + optional strategy chapter + appendix.** This is a short actionable report with fixed structure:
...
5. **Chapter 5 (交易策略板块，可选)**: 由 `render-strategy-section.cjs` 从 `strategy-plan.json` 确定性渲染（策略匹配/入场/止损/目标/仓位/风险评估/执行状态/失效退出/免责声明）；`strategy-plan.json` 缺失或为空时跳过，四章+附录不变
6. **Appendix (价格区间方法说明)**: HV 概率锥 + ATR 通道 + 偏差分析 + 使用建议
...
**验收标准**: 4 章齐全 + 必填字段存在 + 数据质量警告按规则生成，不是行数门禁。第五章（交易策略板块）为可选章节：`strategy-plan.json` 存在且非空时渲染，缺失/为空时跳过且不影响四章验收。
...
|------|------|-----|-----|---------|----------|----------|
| {sector1} | ↑/↓ | +x.x% | +x.x% | xx% | {leader} | 板块驱动（中/低置信）或“成员不足，不判定” |

**板块指标口径**：
- 上涨广度 = 板块内当日上涨成员数 ÷ 成员总数 × 100%（成员当日收益 = 当日收盘/前一收盘 - 1）。
- 广度 ≥ 50% 且与板块方向一致 → 多数成员共振；广度 < 50% 或分化 → 少数品种拉动，不是板块性行情。
...
## 五、交易策略板块（执行参考）（可选章节，由 render-strategy-section.cjs 生成）
...
|------|---------|--------|--------|---------|------|
| {sym} {name} | ↑/↓/→ | 高/中/低 | {strategyId} {strategyName} | {PB-xx} | ✅ 可执行 / 👀 观察 / ⛔ 跳过 |

### {品种}：{名称}（每个 TOP3 一个子节，watch/skip 不省略任何小节）

#### 策略匹配
（matchedStrategies 表：策略/得分/命中证据 + 辅证行 + ≤3 条证据 URL）

#### 执行计划
- 入场（含转执行触发 = Q4 确认价位）/ 执行口径 / 止损（价+距离+依据）/ 目标 T1/T2 / 仓位（手数+依据）

#### 风险评估
（每手风险/止损价/保证金占用/波动率贡献/尾部边距/连续停板压力/事件风险/最长持有）

#### 执行状态与原因
（✅/👀/⛔ 徽标 + statusReasons；watch/skip 附“转执行触发”）

#### 失效与退出
（Q5 硬失效逐条 + 时间止损 + 新 run 取代）

### 免责声明与风险边界

> ⚠️ 免责声明：本板块策略为分析工具输出，仅为方向增强与执行参考，**不构成投资建议，不执行真实交易**。所有参数均为按示例权益的确定性风险计算演示，**不含任何收益承诺或预期收益**；历史文献与样本内回测结论不代表未来表现。
...
9. **交易策略板块（第五章，可选）**: 由 `report/render-strategy-section.cjs` 从 `strategy-plan.json` 确定性渲染，禁止手写/修改其中数值；板块不得改变报告方向与置信度、不得出现收益/胜率承诺；watch/skip 是合法状态，策略适配内容必须完整渲染。
