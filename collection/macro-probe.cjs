// collection/macro-probe.cjs — Phase 3 阶段一：宏观锚点采集
//
...
//
// Usage: node collection/macro-probe.cjs --runId <id>
...
function loadMacroHistoryIndicators(historyPathOverride = null) {
  const p = historyPathOverride || path.join(skillRoot, 'legacy', 'strategies', 'signal-backtest', 'recordings', 'v5', 'macro-history.json');
