...
function normalize(value, prodRunId, mirrorRunId, opts = {}) {
  ...
  if (typeof value === 'string') {
    ...
    s = s.replace(/^> 族级证据状态[^\n]*\n/gm, '');
    // 逐品种可信度是 2026-08-29 promote 的稳定配置新增（历史 run 无 → 两侧归一后比较）
    s = s.replace(/^> 可信度 =[^\n]*\n/gm, '');
    s = s.replace(/^- \*\*实验线可信度\*\*:[^\n]*\n/gm, '');
    s = s.replace(/\| 品种 \| 锚定合约 \| 方向 \| 置信度 \| 主策略\/模板 \| 状态 \| 可信度 \|/g, '| 品种 | 锚定合约 | 方向 | 置信度 | 主策略/模板 | 状态 |');
    s = s.replace(/\|------\|---------\|------\|--------\|-------------\|------\|--------\|/g, '|------|---------|------|--------|-------------|------|');
    s = s.replace(/\| [ABCD] \|$/gm, '|');
    // 前向账本滚动行与板块集中度提示行（历史 run 无 → 两侧删除后比较）
    s = s.replace(/^> 前向验证（上期[^\n]*\n/gm, '');
    s = s.replace(/^> 板块集中度提示：[^\n]*\n/gm, '');
    ...
    s = s.replace(/## 四、交易策略板块（执行参考）/g, '## 四、交易策略');
    s = s.replace(/## 四、今日不做什么\n[\s\S]*?(?=\n---\n)/g, '');
    s = s.replace(/---\n\n\n---/g, '---');
    s = s.replace(/^## 价格区间方法说明\n/gm, '## 五、方法与数据说明\n### 价格区间方法说明\n');
    s = s.replace(/## 五、方法与数据说明\n\n### 价格区间方法说明/g, '## 五、方法与数据说明\n### 价格区间方法说明');
    // 板块异动表新增「方向一致度」列（历史 run 无该列 → 整个板块异动块两侧删除后比较）
    s = s.replace(/### 板块异动\n[\s\S]*?(?=\n## 二、候选筛选)/g, '');
    // 证伪反馈表（新增列：品种/策略；历史 run 无该表 → 两侧删除后比较）
    s = s.replace(/### 上一期策略证伪反馈\n[\s\S]*?(?=\n### |\n## |\n---\n)/g, '');
    // v2 证伪反馈块（近3期明细+历史汇总+口径说明）整块归一删除
    s = s.replace(/### 证伪反馈（近3期明细 \+ 历史汇总）\n[\s\S]*?(?=\n### |\n## |\n---\n)/g, '');
    // 收盘价标注（终稿新增；历史 run 无 → 两侧归一后比较）
    s = s.replace(/（收盘 [\d.,]+）/g, '');
    s = s.replace(/^- \*\*收盘价基准\*\*:[^\n]*\n/gm, '');
    if (s.includes('| # | 品种 | 代码 | 收盘 | 得分 ')) {
      s = s.replace(/\| # \| 品种 \| 代码 \| 收盘 \| 得分 /, '| # | 品种 | 代码 | 得分 ');
      s = s.replace(/\|---\|------\|------\|------\|------\|/, '|---|------|------|------|');
      s = s.replace(/(\| \d+ \| [^|]+ \| [A-Z0-9]+) \| [^|]+ \| /g, '$1 | ');
    }
    // 过滤决策表（初判理由+待验证新增列；历史 run 无 → 两侧归一后比较）
    if (s.includes('| 品种 | 决定 | 初判理由 | 待验证 |')) {
      s = s.replace(/\| 品种 \| 决定 \| 初判理由 \| 待验证 \|/, '| 品种 | 决定 | 理由 |');
      s = s.replace(/\|------\|------\|---------\|--------\|/, '|------|------|------|');
      s = s.replace(/ \| — \|$/gm, ' |');
    }
