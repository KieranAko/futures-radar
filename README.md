...
- **信号质量回测（v0.1.9 → v0.1.10）**：`node strategies/signal-backtest/runner.cjs` 对 RB0/M0/SC0 做“LLM 锚点 → 确定性信号延续 → T+1 确认/T+2 执行 → 证伪”；v1=1 年/10 日锚点（`signal-quality-baseline.md/json`），v2=2 年/5 日锚点并淘汰 v1 最差参数组合（`signal-quality-baseline-2y.md/json`）
...
- **603 个测试全绿**（104 套件），测试夹具内置、无机器路径依赖
