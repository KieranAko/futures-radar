...
- **信号质量回测（v0.1.9 → v0.1.11）**：RB0/M0/SC0 的“LLM 锚点 → 确定性信号延续 → T+1 确认/T+2 执行 → 证伪”；v1=1 年/10 日锚点、v2=2 年/5 日锚点（`runner.cjs`），v3=`runner-v3.cjs` 不做参数选优，改为证伪 LLM 定性判断（regime/edge/triggerType/qualityFlags）并与纯量化 MA20 对照臂比较（输出 `signal-quality-baseline-v3.md/json`）
...
- **608 个测试全绿**（107 套件），测试夹具内置、无机器路径依赖
