...
- **信号质量回测**：`node strategies/signal-backtest/runner.cjs`——固定 RB0/M0/SC0，当前 v2 为 2 年历史（500 交易日）+ 每 5 交易日 LLM 锚点，确定性规则在锚点期内延续生成信号，T+1 收盘确认/T+2 开盘执行/止损/目标/时间退出验证；v1 证伪最差参数组合（trigger0.5×stop1.5×R2×hold6）已淘汰；产出 `strategies/signal-backtest/output/signal-quality-baseline-2y.md/json`（v1 基线保留在 `signal-quality-baseline.md/json`）
